...
class ISMCTSBot(pyspiel.Bot):
  ...
  def resample_from_infostate(self, state):
    possible_places = [index for index, element in enumerate(state.information_state) if element == 1]
    resample_state=state.clone()
    resample_state.people = (random.choice(possible_places),0)
  ...
  # , depth=0, max_depth=100
  def run_simulation(self, state, action_list=[]):
    # if depth > max_depth:
...
def show_policy_converge(prob_list):
  x = list(range(len(prob_list)))
  plt.plot(x, prob_list)

  plt.xlabel('numuber of simulations')
  plt.ylabel('the probability of choosing the optimal action')

  plt.title('Convergence of the ISMCTS policy')
