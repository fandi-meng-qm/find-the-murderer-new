...
class MurderGame(pyspiel.Game):
    def __init__(self, params=None, game_params: MurderParams = None) -> None:
        ...
        n_actions = game_params.m_grid - 1
...
class MurderState(pyspiel.State):
    ...
    def _legal_actions(self, player) -> List[int] or None:
        ...
        assert player >= 0
        init_actions = list(range(self.params.m_grid))
        actions = []
        for i in init_actions:
            if self.information_state[i] == 1:
                actions.append(i)
        actions.pop()
        # print(actions)
        return actions
    ...
    def _apply_action(self, action: int) -> None:
        if self.is_chance_node():
            ...
            if self.people[0] <= action:
                for i in range(action + 1, self.params.m_grid):
